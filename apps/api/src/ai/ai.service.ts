...
// ==========================================
// 資金調達オプション amount のサーバ側サニタイズ
// ==========================================
/** amount は 10 万円単位に丸めて偽の精度を除去する */
export const FUNDING_AMOUNT_STEP = 100_000;
/** 融資シミュ元本として意味を成す下限（100 万円）。これ未満は除外 */
export const FUNDING_AMOUNT_STATIC_MIN = 1_000_000;
/** 財務値が取れないときの静的サニティ上限（3 億円）。超過は除外 */
export const FUNDING_AMOUNT_STATIC_MAX = 300_000_000;
/** 年商が取れるときの上限比率（年商の 50%） */
const FUNDING_REVENUE_CAP_RATIO = 0.5;
...
export interface SanitizedFundingAmount {
  ...
  kept: boolean;
  /** 丸め・クランプ後の金額（kept=false のときは参考値） */
  amount: number;
  reason?: 'non-finite' | 'below-min' | 'above-max' | 'clamped';
}

/**
 * LLM が生成した資金調達 amount を融資シミュ元本へ渡す前にサニタイズする純粋関数。
 *
 * - 10 万円単位に丸めて偽の精度を除去する。
 * - 下限(100 万円)未満は融資元本として無意味なので除外する。
 * - annualRevenue（年商）が渡された場合は「年商の 50% と 1 億円の小さい方」を上限に
 *   クランプ（値を上限まで切り下げてオプションは残す）。
 * - 年商が渡されない/取得できない場合は静的サニティ範囲(100 万〜3 億円)で判定し、
 *   上限超過はクランプせず除外する（財務裏付けなしに大きな元本を残さない）。
 */
export function sanitizeFundingAmount(
  rawAmount: unknown,
  annualRevenue?: number | null,
): SanitizedFundingAmount {
  const n = typeof rawAmount === 'number' ? rawAmount : Number(rawAmount);
  if (!Number.isFinite(n)) return { kept: false, amount: 0, reason: 'non-finite' };

  const rounded = Math.round(n / FUNDING_AMOUNT_STEP) * FUNDING_AMOUNT_STEP;

  // 小さすぎる/負 → 融資元本として無意味 → 除外
  if (rounded < FUNDING_AMOUNT_STATIC_MIN) {
    return { kept: false, amount: rounded, reason: 'below-min' };
  }

  const hasRevenueBasis =
    typeof annualRevenue === 'number' &&
    Number.isFinite(annualRevenue) &&
    annualRevenue > 0;

  if (hasRevenueBasis) {
    const revenueCap = Math.min(
      annualRevenue * FUNDING_REVENUE_CAP_RATIO,
      FUNDING_REVENUE_CAP_ABS,
    );
    const cappedToStep =
      Math.floor(revenueCap / FUNDING_AMOUNT_STEP) * FUNDING_AMOUNT_STEP;
    // 年商が極端に小さくても下限までは調達余地として許容する
    const upper = Math.max(cappedToStep, FUNDING_AMOUNT_STATIC_MIN);
    if (rounded > upper) {
      return { kept: true, amount: upper, reason: 'clamped' };
    }
    return { kept: true, amount: rounded };
  }

  // 財務値の裏付けなし: 静的サニティ範囲外は除外（クランプしない）
  if (rounded > FUNDING_AMOUNT_STATIC_MAX) {
    return { kept: false, amount: rounded, reason: 'above-max' };
  }
  return { kept: true, amount: rounded };
}
...
{"summary":"${targetMonthData.month}単月のエグゼクティブサマリー(3〜5文、具体的な数字を含める)","sections":[${sectionsJsonTemplate}],"highlights":[{"text":"15字以内","type":"positive"},{"text":"15字以内","type":"warning"}]}`;
      ...
      // err.message はユーザー向け summary に出さない。詳細は logger のみ。
      this.logger.error('AI summary generation failed', err?.message);
      return {
        summary:
          'AIサマリーを生成できませんでした。しばらく待って再生成してください。問題が続く場合はMF連携状況をご確認ください。',
        highlights: [],
        targetMonth: targetMonthData.month,
        targetMonthData,
        monthlyTrend: trend,
...
${trendBlock ? '\n' + trendBlock + '\n' : ''}`;
      ...
      // err.message はユーザー向け本文に出さない。詳細は logger のみ。
      this.logger.error('Talk script generation failed', err?.message);
      return {
        opening:
          'トークスクリプトを生成できませんでした。しばらく待って再生成してください。',
        sections: [],
        closing: '',
        generatedAt: new Date().toISOString(),
      };
...
${paramBlock}
## 数値生成の厳禁ルール（厳守）
- revenue / operatingProfit は「今期実績データ」と「指定パラメータ（成長率・採用・コスト削減率）」から算出した妥当な数値のみを出すこと。実績と無関係な数値を捏造しない。
- 与えられていない財務データ（資本金・借入残高等）を勝手に作り出さない。
- 各値は必ず有限な数値（円単位の整数）で出すこと。null・文字列・「未定」等は出さない。
...
[{"name":"Base","description":"基本シナリオの説明","revenue":来期売上予測（円）,"operatingProfit":来期営業利益予測（円）,"assumptions":["前提条件1","前提条件2","前提条件3"]},{"name":"Upside","description":"...","revenue":...,"operatingProfit":...,"assumptions":[...]},{"name":"Downside","description":"...","revenue":...,"operatingProfit":...,"assumptions":[...]}]`;
      ...
      if (!match) return [];
      // LLM 出力をそのまま BudgetScenario[] として返さず、型/有限性を検証して不正値を除外する。
      // 会計プロダクトのため「LLM が生成した壊れた数値」が確定値として UI に出るのを防ぐ。
      return this.safeParseBudgetScenarios(match[0]);
    } catch (err: any) {
  ...
  /**
   * LLM が返した予算シナリオJSON配列を検証する型ガード（drafter の safeParseSections 相当）。
   * name:string / revenue・operatingProfit:有限number / assumptions:string[] を満たすものだけ通す。
   * 不正な要素は黙って除外し、全滅したら空配列を返す（呼び出し側で空配列ハンドリング済み）。
   */
  private safeParseBudgetScenarios(rawJson: string): BudgetScenario[] {
    let parsed: unknown;
    try {
      parsed = JSON.parse(rawJson);
    } catch {
      this.logger.warn('Budget scenarios JSON parse failed');
      return [];
    }
    if (!Array.isArray(parsed)) return [];

    const result: BudgetScenario[] = [];
    for (const item of parsed) {
      if (!item || typeof item !== 'object') continue;
      const o = item as Record<string, unknown>;
      const revenue = typeof o.revenue === 'number' ? o.revenue : Number(o.revenue);
      const operatingProfit =
        typeof o.operatingProfit === 'number' ? o.operatingProfit : Number(o.operatingProfit);
      // 名前と2つの数値が健全でなければ採用しない（捏造・NaN・Infinity を弾く）
      if (typeof o.name !== 'string' || o.name.trim().length === 0) continue;
      if (!Number.isFinite(revenue) || !Number.isFinite(operatingProfit)) continue;
  ...
  /**
   * 資金調達レポートの locabenImprovements を「サーバ決定論算出値」で裏打ちするための
   * 指標テーブルを返す。current / benchmark / gap は computeLocabenMetrics と業種ベンチマークから
   * 算出済みの確定値であり、LLM に作らせてはいけない。getLocabenBlock と同じ計算を共有する。
   * 失敗時（MF 未接続など）は空配列。
   */
  private async getLocabenMetricRows(
    orgId: string,
    fiscalYear?: number,
    endMonth?: number,
    locabenOverride?: {
    ...
    Array<{ metricLabel: string; current: string; benchmark: string; gap: string; belowBenchmark: boolean }>
  > {
    try {
      const org = await this.prisma.organization.findUnique({
        where: { id: orgId },
      ...
      const industry = normalizeIndustry(locabenOverride?.industry ?? org?.industry);
      const mfSource = await this.locabenService.getSourceData(orgId, fiscalYear, endMonth);
      const sourceData = { ...mfSource };
      if (locabenOverride?.values) {
        for (const key of Object.keys(sourceData) as (keyof typeof sourceData)[]) {
          const v = locabenOverride.values[key as string];
      ...
      for (const key of LOCABEN_METRIC_KEYS) {
        ...
        if (v !== null && Number.isFinite(v)) {
          const diff = v - b;
          gap = `${diff >= 0 ? '+' : ''}${diff.toFixed(1)}`;
          // 「劣後している」= higher_is_better なのに平均未満 / lower_is_better なのに平均超
          belowBenchmark = higher ? diff < 0 : diff > 0;
        }
        rows.push({
          metricLabel: LOCABEN_METRIC_LABELS[key],
          current: fmt(v),
          benchmark: b.toFixed(1),
          gap,
          belowBenchmark,
        });
      }
      ...
      this.logger.warn(
        `Locaben metric rows skipped: ${err instanceof Error ? err.message : err}`,
      );
      return [];
    }
  }
  ...
  async generateFundingReport(
    ...
    try {
      ...
      // ロカベン6指標の current/benchmark/gap はサーバ決定論算出値。
      // これを正本としてプロンプトに注入し、parse 後に LLM 出力の数値を上書き／棄却する。
      const locabenRows = await this.getLocabenMetricRows(
        orgId,
        fiscalYear,
        endMonth,
        locabenOverride,
      );
      const locabenRowByLabel = new Map(locabenRows.map((r) => [r.metricLabel, r]));
      const periodNote = endMonth
...
${scenarioBlock}
## データ捏造の厳禁（厳守）
- 与えられた財務データ・ロカベン指標表・検討中シナリオに無い金額を絶対に作り出さない。引用する数値は上記で与えられたものに限る。
- ロカベン改善提案の current（実績）/ benchmark（業種平均）/ gap（差分）は、上記「ロカベン分析」の表に記載された確定値をそのまま転記すること。**自分で再計算したり別の数値を書いたりしない。**
- suggestedOptions の amount（調達想定額）は、現預金水準・ランウェイ・検討中シナリオの元金から導ける現実的なレンジに留める。根拠なく大きな数値を作らない。
- 与えられていない財務データ（簿外債務・含み益等）を前提にした金額を書かない。
...
{"executiveSummary":"事業概要と資金ニーズ（3-5文）","financialHighlights":["財務ハイライト1","ハイライト2","ハイライト3"],"strengthsRisks":{"strengths":["強み1","強み2"],"risks":["リスク1","リスク2"]},"projections":"今後の見通し（3-5文）","suggestedOptions":[{"type":"銀行借入(運転資金)","amount":30000000,"rationale":"...","suggestedRate":2.5,"suggestedMonths":60,"repaymentType":"EQUAL_INSTALLMENT"}],"locabenImprovements":[{"metricLabel":"自己資本比率(%)","current":"22.0","benchmark":"40.0","gap":"-18.0","priority":"high","suggestion":"内部留保の積み増しを最優先。当期利益の社外流出（役員報酬・配当）を△500万円抑え、5年で自己資本比率35%まで回復させる計画を提示する。"}]}`;
      ...
      // ロカベン改善提案の数値はサーバ決定論算出値で上書きする。
      // LLM が current/benchmark/gap を生成・改変していても、計算済みの確定値で置き換え、
      // 既知指標と一致しない（=LLM が捏造した）行は棄却する。文章(suggestion)のみ LLM 由来を残す。
      const reconciledLocaben = this.reconcileLocabenImprovements(
        parsed?.locabenImprovements,
        locabenRowByLabel,
      );

      // suggestedOptions の amount をサーバ側でサニタイズ。年商ベースの上限クランプ +
      // 10 万円丸め + 範囲外除外で、LLM 生値が融資シミュ元本にそのまま流れるのを防ぐ。
      const annualRevenue = this.annualizeRevenue(dashboard?.revenue, endMonth);
      const safeOptions = this.sanitizeFundingOptions(
      ...
      return {
        ...
        suggestedOptions: safeOptions,
        locabenImprovements: reconciledLocaben,
        echoedScenarios: scenarios,
        generatedAt: new Date().toISOString(),
      };
    } catch (err: any) {
      // err.message はユーザー向け本文に混入させない（内部実装/接続先が漏れるため）。詳細は logger のみ。
      this.logger.error('Funding report generation failed', err?.message);
      return {
        executiveSummary:
          '資金調達レポートを生成できませんでした。MF会計の連携状況を確認のうえ、しばらくして再生成してください。',
        financialHighlights: [],
  ...
  /**
   * LLM が出した locabenImprovements の数値（current/benchmark/gap）をサーバ算出値で上書きし、
   * サーバ側の既知指標（serverRows）と metricLabel が一致しない行は捨てる。
   * priority と suggestion（散文）のみ LLM 由来を保持する。serverRows が空なら検証材料がないため空配列。
   */
  private reconcileLocabenImprovements(
    raw: LocabenImprovement[] | undefined,
    serverRows: Map<
      string,
      { metricLabel: string; current: string; benchmark: string; gap: string; belowBenchmark: boolean }
    >,
  ): LocabenImprovement[] {
    if (!Array.isArray(raw) || serverRows.size === 0) return [];
    const result: LocabenImprovement[] = [];
    const validPriorities = new Set(['high', 'medium', 'low']);
    for (const item of raw) {
      if (!item || typeof item !== 'object') continue;
      const label = typeof item.metricLabel === 'string' ? item.metricLabel.trim() : '';
      const row = serverRows.get(label);
      // サーバが算出していない指標ラベルを LLM が作り出した場合は棄却（捏造防止）
      if (!row) continue;
      const priority = validPriorities.has(item.priority as string)
        ? (item.priority as LocabenImprovement['priority'])
        : 'medium';
      result.push({
        metricLabel: row.metricLabel,
        current: row.current, // サーバ算出値で上書き
        benchmark: row.benchmark, // サーバ算出値で上書き
        gap: row.gap, // サーバ算出値で上書き
        priority,
  ...
  /**
   * suggestedOptions の数値フィールドを検証・サニタイズする。
   * amount は sanitizeFundingAmount で丸め・上限クランプ・範囲外除外し、除外/クランプは
   * ログに残す（LLM 生値がそのまま融資シミュ元本に流れるのを防ぐ）。
   * suggestedRate / suggestedMonths は有限でなければ undefined に落とす。
   *
   * @param annualRevenue 年商（サーバ算出値）。渡されると amount 上限を財務値ベースにする。
   */
  private sanitizeFundingOptions(
    raw: FundingOption[] | undefined,
    annualRevenue?: number | null,
  ): FundingOption[] {
    if (!Array.isArray(raw)) return [];
    const validRepayment = new Set(['EQUAL_INSTALLMENT', 'EQUAL_PRINCIPAL', 'BULLET']);
    const result: FundingOption[] = [];
    for (const item of raw) {
      if (!item || typeof item !== 'object') continue;
      if (typeof item.type !== 'string' || item.type.trim().length === 0) continue;
      const sanitized = sanitizeFundingAmount(item.amount, annualRevenue);
      if (!sanitized.kept) {
        this.logger.warn(
          `Funding option dropped: type=${item.type} rawAmount=${String(item.amount)} reason=${sanitized.reason}`,
        );
        continue;
      }
      if (sanitized.reason === 'clamped') {
        this.logger.warn(
          `Funding option amount clamped: type=${item.type} rawAmount=${String(item.amount)} -> ${sanitized.amount}`,
        );
      }
      const rate = Number(item.suggestedRate);
      const months = Number(item.suggestedMonths);
      result.push({
        type: item.type.trim(),
        amount: sanitized.amount,
        rationale: typeof item.rationale === 'string' ? item.rationale : '',
        suggestedRate: Number.isFinite(rate) ? rate : undefined,
        suggestedMonths: Number.isFinite(months) ? months : undefined,
        repaymentType: validRepayment.has(item.repaymentType as string)
          ? item.repaymentType
          : undefined,
      });
    }
  ...
  /**
   * 資金調達 amount 上限の基準となる年商を求める。dashboard.revenue は集計期間
   * (期首〜endMonth) の累計なので、期中(endMonth<12)なら月商換算で年換算する。
   * 売上が 0/非数のときは null（= 財務値ベースの上限を使わず静的サニティに委ねる）。
   */
  private annualizeRevenue(
    periodRevenue: number | undefined,
    endMonth?: number,
  ): number | null {
    if (typeof periodRevenue !== 'number' || !Number.isFinite(periodRevenue) || periodRevenue <= 0) {
      return null;
    }
    if (endMonth && endMonth >= 1 && endMonth < 12) {
      return (periodRevenue / endMonth) * 12;
    }
